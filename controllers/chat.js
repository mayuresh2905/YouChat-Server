...
import { deleteFilesfromCloudinary, emitEvent, uploadsFilesToCloudinary } from "../utils/features.js";
...
export const addMembers = TryCatch( async(req,res,next) => {
   ...
   if (!chat.groupChat) return next(new ErrorHandler("This not a group",404));
...
export const removeMember = TryCatch( async (req,res,next) => {
   ...
   const [chat, userThatWillBrRemoved] = await Promise.all([
   ...
   emitEvent(req,ALERT,chat.members,{ message:`${userThatWillBrRemoved.name} has been removed from the group`,chatId});
   emitEvent (req,REFETCH_CHATS,allChatMembers);

   return res.status(200).json({
      success: true,
      members: "Member removed sucessfully", 
   })
});
...
export const sendAttachments = TryCatch( async (req,res,next) => {
   ...
   // Upload Files Here
   const attachments = await uploadsFilesToCloudinary(files);
